...
function Registeration_Form() {
  ...
  const onSubmit = async (e) => {
    ...
    if (
      ...
      await fetch("http://192.168.10.5:9000/pick-and-drop-registration", {
        method: "POST",
        headers: {
...
leftIcon={
  <Icon style={{marginTop: windowHeight/18,marginLeft: windowWidth/18}}
  name='user'
  size={18}
  color='black'
  />
...
<Input style={{marginLeft: 15}}  placeholder="Enter address" on onChangeText={(text)=> set_address(text)}
leftIcon={
...
<Input style={{marginLeft: 15}}  placeholder="Enter Charges" onChangeText={(text) => set_charges(text)} keyboardType = "numeric"
leftIcon={
